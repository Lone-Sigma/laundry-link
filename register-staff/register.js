...
document.getElementById("registerBtn").addEventListener("click", async (e) => {
  ...
  try {
    const dbRef = ref(db);
    const existing = await get(child(dbRef, `staffs/${staffID}`)); // ✔ check under 'staffs/'
    ...
    const dateRegistered = getCurrentDateTime();

    // Save to Realtime Database
    await set(ref(db, `staffs/${staffID}`), {
      fullName: fullName,
      contact: contact,
      password: hashedPassword,
      dateRegistered: dateRegistered,
      status: "active" // ✔ set as 'inactive' on register
    });
